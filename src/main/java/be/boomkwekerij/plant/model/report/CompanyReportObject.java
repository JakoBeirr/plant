package be.boomkwekerij.plant.model.report;

public class CompanyReportObject {

    private String name1;
    private String name2;
    private String address;
    private String telephoneBelgium;
    private String telephoneNetherlands;
    private String faxBelgium;
    private String faxNetherlands;
    private String gsmBelgium;
    private String gsmNetherlands;
    private String ibanBelgium;
    private String bicBelgium;
    private String ibanNetherlands;
    private String bicNetherlands;
    private String btwNumber;
    private String emailAddress;

    public String getName1() {
        return name1;
    }

    public void setName1(String name1) {
        this.name1 = name1;
    }

    public String getName2() {
        return name2;
    }

    public void setName2(String name2) {
        this.name2 = name2;
    }

    public String getAddress() {
        return address;
    }

    public void setAddress(String address) {
        this.address = address;
    }

    public String getTelephoneBelgium() {
        return telephoneBelgium;
    }

    public void setTelephoneBelgium(String telephoneBelgium) {
        this.telephoneBelgium = telephoneBelgium;
    }

    public String getTelephoneNetherlands() {
        return telephoneNetherlands;
    }

    public void setTelephoneNetherlands(String telephoneNetherlands) {
        this.telephoneNetherlands = telephoneNetherlands;
    }

    public String getFaxBelgium() {
        return faxBelgium;
    }

    public void setFaxBelgium(String faxBelgium) {
        this.faxBelgium = faxBelgium;
    }

    public String getFaxNetherlands() {
        return faxNetherlands;
    }

    public void setFaxNetherlands(String faxNetherlands) {
        this.faxNetherlands = faxNetherlands;
    }

    public String getGsmBelgium() {
        return gsmBelgium;
    }

    public void setGsmBelgium(String gsmBelgium) {
        this.gsmBelgium = gsmBelgium;
    }

    public String getGsmNetherlands() {
        return gsmNetherlands;
    }

    public void setGsmNetherlands(String gsmNetherlands) {
        this.gsmNetherlands = gsmNetherlands;
    }

    public String getIbanBelgium() {
        return ibanBelgium;
    }

    public void setIbanBelgium(String ibanBelgium) {
        this.ibanBelgium = ibanBelgium;
    }

    public String getBicBelgium() {
        return bicBelgium;
    }

    public void setBicBelgium(String bicBelgium) {
        this.bicBelgium = bicBelgium;
    }

    public String getIbanNetherlands() {
        return ibanNetherlands;
    }

    public void setIbanNetherlands(String ibanNetherlands) {
        this.ibanNetherlands = ibanNetherlands;
    }

    public String getBicNetherlands() {
        return bicNetherlands;
    }

    public void setBicNetherlands(String bicNetherlands) {
        this.bicNetherlands = bicNetherlands;
    }

    public String getBtwNumber() {
        return btwNumber;
    }

    public void setBtwNumber(String btwNumber) {
        this.btwNumber = btwNumber;
    }

    public String getEmailAddress() {
        return emailAddress;
    }

    public void setEmailAddress(String emailAddress) {
        this.emailAddress = emailAddress;
    }
}
